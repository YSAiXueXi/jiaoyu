...
Page({
  ...
  //修改登录方式
  loginMethodChange(e) {
    let loginMethod = e.currentTarget.dataset.text;
  ...
  passwordLoginSumbit(e){
    ...
    if (accountNumber == '') {
      ...
      this.loginSuccessSwitchPage(accountNumber);
    } else {
  ...
  // 处理跳转，如果该账户是多个角色，就跳转切换角色页面
  // 否则，该职业是老师，跳转老师端首页
  // 否则，跳转首页
  loginSuccessSwitchPage(accountNumber){
    let roleList = data.getRoleList(accountNumber);
    if (roleList.leng > 1) {
      wx.navigateTo({
        url: '/pages/switch_roles/roles',
      })
  ...
  reallyVerCode(reallyVerCode) {
    this.setData({
      reallyVerCode: reallVerCode.detail
    })
  },
  ...
  verCodeLogin() {
    ...
    if (data.hasAccount(accountNumber) && this.data.reallyVerCode == this.data.verficationCode) {
      wx.setStorage({
        key: 'account',
        data: new Account(accountNumber, '')
      })
