...
Page({
  ...
  choose(e){
    ...
    wx.navigateTo({
      url: "/pages/client_teacher/choose/choose?title=" + title
    })
  },
  ...
  onGoSignature(){
    wx.navigateTo({
      url: "/pages/client_teacher/feedback/feedback"
    })
  },
  ...
  onTakePhoto(){
    this.onHTPC();
    wx.navigateTo({
      url: "/pages/tackphoto/tackphoto"
    })
  },
  // 延时方法（把照片路径绑定到数组）
  //发觉不延时，图片路径会自动取消，没搞清楚原因
  onDelayedEvent(tempFilePath){
    ...
    } else if (tempFilePath.constructor.toString().indexOf("String") != -1) {
  ...
  //添加多张照片
  onAddPhotos(photos){
    let photoArray = this.data.photoArrau;
    photos.forEach(item => {
  ...
  onGetFromAlbum(){
    ...
    wx.chooseImage({
      ...
      success: function(res){
        that.onDelayedEvent(res.tempFilePaths);
      },
      fail(err){
        console.log("从相册获取失败!");
      }
